<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Catálogo de Cuentas de Egreso</h1>
                <p class="text-sm text-slate-500">Nomenclatura contable para el registro y clasificación de gastos y desembolsos.</p>
            </div>
            @can('catalogos.gestionar')
                <x-ui.button href="{{ route('catalogos.egresos.create') }}" variant="primary">
                    <svg class="w-4 h-4 me-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                    Nueva Cuenta de Egreso
                </x-ui.button>
            @endcan
        </div>
    </x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="border-b border-slate-200">
            <nav class="-mb-px flex space-x-6">
                <a href="{{ route('catalogos.ingresos.index') }}" class="border-b-2 border-transparent pb-3 px-1 text-sm font-medium text-slate-500 hover:text-slate-700 hover:border-slate-300">
                    Cuentas de Ingreso
                </a>
                <a href="{{ route('catalogos.egresos.index') }}" class="border-b-2 border-primary-600 pb-3 px-1 text-sm font-semibold text-primary-600">
                    Cuentas de Egreso
                </a>
            </nav>
        </div>
        ...
        <!-- Filtros y Búsqueda -->
        <x-ui.card>
            <form method="GET" action="{{ route('catalogos.egresos.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input
                        ...
                        placeholder="Ej. 001 o Combustible"
                ...
                <div class="flex items-end gap-2">
                    ...
                    @if (request()->hasAny(['buscar', 'estado']))
                        <a href="{{ route('catalogos.egresos.index') }}" class="inline-flex items-center justify-center p-2 text-slate-500 hover:text-slate-800" title="Limpiar filtros">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            @can('catalogos.gestionar')
                                ...
                                @else
                                    <a href="{{ route('catalogos.egresos.edit', $cuenta) }}" class="text-xs font-medium text-primary-600 hover:text-primary-800">
                                        Editar
                                    </a>

                                    <form method="POST" action="{{ route('catalogos.egresos.estado', $cuenta) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button
                                            type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-10 text-center text-sm text-slate-500">
                            No se encontraron cuentas de egreso registradas con los filtros indicados.
                        </td>
                    </tr>
                @endforelse
            </tbody>
