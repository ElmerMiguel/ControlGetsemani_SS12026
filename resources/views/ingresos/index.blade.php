<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Ingresos y Aportaciones</h1>
                <p class="text-sm text-slate-500">Registro y consulta de diezmos, ofrendas y donaciones recibidas.</p>
            </div>
            @can('ingresos.crear')
                <x-ui.button href="{{ route('ingresos.create') }}" variant="primary">
    ...
    <div class="space-y-6" x-data="{ anularModalOpen: false, anularAction: '', anularId: '' }">
        <!-- Filtros y Búsqueda -->
        <x-ui.card>
            <form method="GET" action="{{ route('ingresos.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div>
                ...
                <div>
                    <label for="cuenta_id" class="block text-xs font-medium text-slate-600 mb-1">Cuenta de Ingreso</label>
                    <select
                        name="cuenta_id"
                        id="cuenta_id"
                        class="w-full text-sm rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500"
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="Recibo, aportante u obs..."
                        class="w-full text-sm rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500"
                    />
                </div>
                ...
                <div class="md:col-span-5 flex flex-wrap items-center justify-between gap-4 pt-2 border-t border-slate-100">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if (request()->hasAny(['caja_id', 'cuenta_id', 'fecha_desde', 'fecha_hasta', 'buscar', 'anulados']))
                            <a href="{{ route('ingresos.index') }}" class="inline-flex items-center justify-center p-2 text-slate-500 hover:text-slate-800" title="Limpiar filtros">
        ...
        <!-- Tabla de Ingresos con Fila de Total SQL -->
        <x-ui.table>
            <thead class="bg-slate-50 text-xs font-semibold uppercase text-slate-500 border-b border-slate-200">
                <tr>
                    ...
                    <th scope="col" class="px-4 py-3.5">Recibo</th>
                    <th scope="col" class="px-4 py-3.5">Aportante</th>
            ...
            <tbody class="divide-y divide-slate-200 bg-white">
                @forelse ($ingresos as $ingreso)
                    @php
                        $estaBloqueado = $ingreso->esBloqueado();
                        $estaAnulado = $ingreso->trashed();
                    @endphp
                    <tr class="hover:bg-slate-50 transition-colors {{ $estaAnulado ? 'bg-red-50/50 opacity-75' : '' }}">
                        ...
                        <td class="px-4 py-4 text-sm text-slate-800">
                            <span class="font-mono text-xs text-slate-500">{{ $ingreso->cuenta->codigo }}</span>
                            {{ $ingreso->cuenta->nombre }}
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap text-xs font-mono text-slate-600">
                            {{ $ingreso->recibo ?? '—' }}
                        </td>
                        <td class="px-4 py-4 text-sm text-slate-700">
                            {{ $ingreso->aportante?->nombre_completo ?? '—' }}
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap text-right font-mono text-sm font-bold tabular-nums {{ $estaAnulado ? 'text-red-700 line-through' : 'text-emerald-600' }}">
                        ...
                        <td class="px-4 py-4 whitespace-nowrap text-right space-x-2">
                            @if ($estaAnulado)
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800" title="Motivo: {{ $ingreso->motivo_anulacion }}">
                                    Anulado
                                </span>
                            ...
                            @else
                                @can('update', $ingreso)
                                    <a href="{{ route('ingresos.edit', $ingreso) }}" class="text-xs font-medium text-primary-600 hover:text-primary-800">
                                        Editar
                                    </a>
                                @endcan
                                ...
                                @can('anular', $ingreso)
                                    <button
                                        type="button"
                                        @click="anularModalOpen = true; anularId = '{{ $ingreso->id }}'; anularAction = '{{ route('ingresos.anular', $ingreso) }}'"
                ...
                @empty
                    <tr>
                        <td colspan="9" class="px-6 py-12 text-center text-sm text-slate-500">
                            No se encontraron ingresos registrados con los criterios de búsqueda.
                        </td>
                    </tr>
                @endforelse
            </tbody>
            ...
            <tfoot class="bg-slate-50 border-t-2 border-slate-300 font-bold text-slate-900">
                <tr>
                    ...
                    <td class="px-4 py-3.5 text-right font-mono text-base text-emerald-700 tabular-nums">
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                >
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-lg font-bold text-slate-900">Anular Ingreso Contable</h3>
                    ...
                    <form :action="anularAction" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                required
                                minlength="10"
                                placeholder="Indica detalladamente por qué se anula este ingreso..."
