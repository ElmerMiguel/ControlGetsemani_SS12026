...
\begin{document}
	...
	\paragraph{Misión} Administrar con transparencia, fidelidad y orden los recursos financieros y bienes materiales encomendados a la Iglesia Getsemaní, facilitando la rendición de cuentas de cada departamento ante la Junta Directiva y la Asamblea General.
	...
	\paragraph{Valores institucionales que guían el diseño del sistema:}
	\begin{itemize}
	...
	Actualmente, la tesorería de cada consejo, comité y congregación anexa lleva sus registros en hojas de cálculo independientes (\texttt{PorConsejo\_o\_comite.xlsx}), las cuales luego son transcritas manualmente a un archivo consolidado (\texttt{InformeGeneral.xlsx}) para su presentación en asambleas.
	...
	\begin{center}
		\begin{tikzpicture}[node distance=1cm, box/.style={rectangle, draw=primary, fill=cardbg, rounded corners, font=\small\bfseries, align=center, inner sep=6pt}]
			...
			\node[box, below=0.6cm of rm] (ra) {Reporte Anual};
			\node[box, below=0.6cm of ra] (ig) {Informe General de Asamblea};
	...
	\begin{center}
		\begin{tikzpicture}[node distance=1.2cm, box/.style={rectangle, draw=primary, fill=cardbg, rounded corners, font=\small\bfseries, align=center, inner sep=6pt}]
			\node[box] (cli) {[Navegador del Tesorero / Admin]};
			\node[box, right=of cli] (srv) {[Servidor Web Laravel (Blade)]};
			\node[box, right=of srv] (db) {[Base de Datos MySQL Cloud]};
			
			\draw[->, >=stealth, thick] (cli) -- node[above, font=\tiny] {HTTPS} (srv);
			\draw[->, >=stealth, thick] (srv) -- node[above, font=\tiny] {Eloquent ORM} (db);
		\end{tikzpicture}
	\end{center}
	...
	\begin{itemize}
		\item \textbf{Jira:} Backlog estructurado en Historias de Usuario con Sprints de 3 semanas.
		\item \textbf{Estrategia GitFlow:} Rama \texttt{main} para versiones estables, \texttt{develop} para integración y ramas \texttt{feature/...} por cada módulo.
	\end{itemize}
